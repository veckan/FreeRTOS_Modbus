...
void gpio_init()
{
	...
	RCC_AHB1PeriphClockCmd(TIPKA_GORE_RCC_GPIOx, ENABLE);
	GPIO_InitStruct.GPIO_Pin		= TIPKA_GORE_PinNumber;
	GPIO_InitStruct.GPIO_Mode		= GPIO_Mode_IN;
	GPIO_InitStruct.GPIO_Speed	= GPIO_Speed_50MHz;
	GPIO_InitStruct.GPIO_OType	= GPIO_OType_PP;
	GPIO_InitStruct.GPIO_PuPd		= GPIO_PuPd_DOWN;
	GPIO_Init(TIPKA_GORE_GPIOx, &GPIO_InitStruct);
	
	RCC_AHB1PeriphClockCmd(TIPKA_DOLJE_RCC_GPIOx, ENABLE);
	GPIO_InitStruct.GPIO_Pin		= TIPKA_DOLJE_PinNumber;
	GPIO_Init(TIPKA_DOLJE_GPIOx, &GPIO_InitStruct);
	
	RCC_AHB1PeriphClockCmd(MOTOR_GORE_RCC_GPIOx, ENABLE);
	GPIO_InitStruct.GPIO_Pin		= MOTOR_GORE_PinNumber;
	GPIO_InitStruct.GPIO_Mode		= GPIO_Mode_OUT;
	GPIO_InitStruct.GPIO_Speed	= GPIO_Speed_50MHz;
	GPIO_InitStruct.GPIO_OType	= GPIO_OType_PP;
	GPIO_InitStruct.GPIO_PuPd		= GPIO_PuPd_NOPULL;
	GPIO_Init(MOTOR_GORE_GPIOx, &GPIO_InitStruct);
	
	RCC_AHB1PeriphClockCmd(MOTOR_DOLJE_RCC_GPIOx, ENABLE);
	GPIO_InitStruct.GPIO_Pin		= MOTOR_DOLJE_PinNumber;
	GPIO_Init(MOTOR_DOLJE_GPIOx, &GPIO_InitStruct);
}
